...
TEST(TyrTests, TyrGrounderGenerator)
{
    ...
    auto fact_sets = grounder::FactSets<Repository> { program.get_atoms<formalism::FluentTag>(), program.get_function_values<formalism::FluentTag>() };
    ...
    assignment_sets.fluent_sets.predicate.insert(fact_sets.predicate);
    assignment_sets.fluent_sets.function.insert(fact_sets.function);
