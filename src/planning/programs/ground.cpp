                                                                           ...
                                                                           MergeCache<OverlayRepository<Repository>, Repository>& merge_cache)
{
    ...
    append_from_condition(action.get_condition(), builder, repository, merge_cache, conj_cond);
                                                                           ...
                                                                           MergeCache<OverlayRepository<Repository>, Repository>& merge_cache)
{
    ...
    append_from_condition(action.get_condition(), builder, repository, merge_cache, conj_cond);
    append_from_condition(cond_eff.get_condition(), builder, repository, merge_cache, conj_cond);
                                                  ...
                                                  GroundTaskProgram::RuleToActionsMapping& rule_to_actions_mapping)
{
    ...
    if (instantiated_through_literals)
        for (const auto cond_eff : action.get_effects())
        {
            ...
            auto body = make_delete_free_body(action, cond_eff, builder, repository, merge_cache);
            ...
            for (const auto literal : cond_eff.get_effect().get_literals())
            {
                ...
                rule.head = merge(literal.get_atom(), builder, repository, merge_cache).get_index();
    ...
    else
    {
        ...
        auto body = make_delete_free_body(action, builder, repository, merge_cache);
                                                                   ...
                                                                   MergeCache<OverlayRepository<Repository>, Repository>& merge_cache)
{
    ...
    process_delete_free_axiom_body(axiom.get_body(), builder, repository, merge_cache, conj_cond);
                                               ...
                                               Repository& repository)
{
    ...
    for (const auto action : task.get_task().get_domain().get_actions())
        translate_action_to_delete_free_rules(action, program, repository, builder, merge_cache, rule_to_actions_mapping);

    for (const auto axiom : task.get_task().get_domain().get_axioms())
    {
        auto new_rule = create_delete_free_axiom_rule(axiom, builder, repository, merge_cache);
