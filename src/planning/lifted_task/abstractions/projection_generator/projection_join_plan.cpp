                                      ...
                                      const StaticAtomIndex& static_index)
{
    ...
    plan.precondition = build_condition_join_plan(action.condition, action.num_variables, static_index);
    ...
    for (const auto& ceff : action.effects)
    {
        ...
        auto cond_plan = build_condition_join_plan(ceff.condition, total_params, static_index);
                            ...
                            const StaticAtomIndex& static_index)
{
    ...
    for (const auto& [projected_action, info] : projected_to_original_action)
    {
        ...
        result.emplace(projected_action, build_action_join_plan(mutable_action, pattern, static_index));
    }
