...
`list[T]` validates every element against `T`. `set[K]` is encoded as a JSON
array with unique semantic elements and validates every element against `K`.
`dict[K, V]` validates every JSON object key against `K` and every value against
`V`. Composite types can be nested, for example `list[dict[str, set[int]]]`.
...
Comparison follows Python value semantics after type normalization. Lists compare
in order, sets compare without order and support subset/superset ordering, and
dicts compare by JSON object contents. If Python does not support the requested
ordered comparison for a value type, the comparison status is
`unsupported_compare`.
