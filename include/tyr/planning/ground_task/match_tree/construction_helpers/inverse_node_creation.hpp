...
#ifndef TYR_PLANNING_GROUND_TASK_MATCH_TREE_CONSTRUCTION_HELPERS_INVERSE_NODE_CREATION_HPP_
#define TYR_PLANNING_GROUND_TASK_MATCH_TREE_CONSTRUCTION_HELPERS_INVERSE_NODE_CREATION_HPP_
...
bool contains_positive(formalism::GroundAtom<P> atom, const E* element)
{
    ...
    const auto& positive_precondition = conjunctive_condition->template get_precondition<formalism::PositiveTag, P>();
...
{
    const auto& conjunctive_condition = element->get_conjunctive_condition();
    const auto& negative_precondition = conjunctive_condition->template get_precondition<formalism::NegativeTag, P>();
