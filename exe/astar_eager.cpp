...
int main(int argc, char** argv)
{
    ...
    program.add_argument("-H", "--heuristic-type")
        .default_value("blind")
        .choices("blind", "goal_count", "rpg_max", "rpg_add", "rpg_ff", "canonical", "projection_abstraction_first");
    ...
    auto total_time = std::chrono::nanoseconds { 0 };
    {
        ...
        if (!instantiate_ground_task)
        {
            ...
            else if (heuristic_type == "projection_abstraction_first")
            {
                auto time_pattern_generator = std::chrono::nanoseconds { 0 };
                auto time_projection_generator = std::chrono::nanoseconds { 0 };
                auto time_projection_heuristic = std::chrono::nanoseconds { 0 };
                ...
                if (projections.empty())
                    throw std::runtime_error("No projections were generated for projection_abstraction_first.");

                {
                    auto stop_watch = StopwatchScope(time_projection_heuristic);

                    heuristic = planning::ProjectionAbstractionHeuristic<planning::LiftedTag>::create(projections.front());
                }
                std::cout << "[Total] First projection heuristic time: " << to_ms(time_projection_heuristic) << " ms" << std::endl;
            }
            else
