...
int main(int argc, char** argv)
{
    ...
    auto total_time = std::chrono::nanoseconds { 0 };
    {
        ...
        for (const auto& rule_ws : successor_generator.get_workspace().rules_iter)
        ...
        for (const auto& rule_ws : successor_generator.get_state_repository()->get_axiom_evaluator()->get_workspace().rules_iter)
        ...
        for (const auto& rule_ws : ff_heuristic->get_workspace().rules_iter)
